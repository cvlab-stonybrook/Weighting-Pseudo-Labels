...
class FeatureMemory:
    def __init__(self, num_samples, dataset,  memory_per_class=2048, feature_size=256, n_classes=19):
        self.num_samples = num_samples#10490
        self.memory_per_class = memory_per_class#256
        ...
        elif dataset == 'pascal': # usually only around 3 classes on each image, except background class
            self.per_class_samples_per_image = max(1, int(n_classes / 3 * round(memory_per_class / num_samples)))#1
    ...
    def add_features_from_sample_learned(self, model, features, class_labels, batch_size):
        ...
        for c in range(self.n_classes):
            ...
            #selector = model.module.__getattr__('contrastive_class_selector_' + str(c))  # get the self attention moduel for class c
            features_c = features[mask_c, :] # get features from class c#[3665, 256]
            if features_c.shape[0] > 0:
                #print(features_c.shape[0])
...
#                         indices = indices.cpu().numpy()
                        ...
                        indices = random.sample(list(range(features_c.shape[0])),elements_per_class)
