...
#         for class_c in range(cfg['nclass']):
...
#             self.__setattr__('contrastive_class_selector_' + str(class_c), selector)
    ...
    def forward(self, x, need_fp=False, need_feat=False):
        ...
        if need_fp:
            if(need_feat):
                features, outs = self._decode(torch.cat((c1, nn.Dropout2d(0.5)(c1))),
                                    torch.cat((c4, nn.Dropout2d(0.5)(c4))), need_feat)#[32, 256, 81, 81]) ([32, 21, 81, 81]
                outs = F.interpolate(outs, size=(h, w), mode="bilinear", align_corners=True)
                features = F.interpolate(features, size=(h, w), mode="bilinear", align_corners=True)#[32, 256, 321, 321]) 
                out, out_fp = outs.chunk(2)
                feature, feature_fp = features.chunk(2)
                return feature, feature_fp,out, out_fp                
            ...
            return out, out_fp
        if(need_feat):
            features,out = self._decode(c1, c4,need_feat)
    ...
    def _decode(self, c1, c4, need_feat=False):
        c4 = self.head(c4)
